...
const ProductAccountCard = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = () => {
    dispatch(productsThunks.deleteItem(item.id))
      .unwrap()
      .then(() => {
  ...
  return (
    <>
      <div className="bg-white shadow-md p-4 rounded-lg cursor-pointer hover:shadow-lg transition relative group">
        ...
        <p className="text-green-600 font-bold">{item.price} DT</p>
...
export default ProductAccountCard;
