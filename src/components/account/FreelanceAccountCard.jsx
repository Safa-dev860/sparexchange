...
import { freelanceThunks } from "../../redux/slices/categorySlice"; // Adjust the import based on your Redux setup

const FreelanceAccountCard = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = () => {
    dispatch(freelanceThunks.deleteItem(item.id)) // Adjust the thunk name as per your setup
      .unwrap()
      .then(() => {
        console.log(`Gig ${item.id} deleted successfully`);
  ...
  return (
    <>
      <div className="bg-white shadow-md p-4 rounded-lg cursor-pointer hover:shadow-lg transition relative group">
        ...
        {/* Uncomment and adjust if you want to show package details */}
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg flex items-center justify-center gap-4">
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(`/freelance/${item.id}/conversations`);
            }}
            title="Conversations"
          >
            <svg
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <p className="mb-6">
              Are you sure you want to delete "{item.gigTitle}"?
            </p>
...
export default FreelanceAccountCard;
