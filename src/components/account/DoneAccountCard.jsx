...
import { doneThunks } from "../../redux/slices/categorySlice"; // Adjust the import path as needed

const DoneAccountCard = ({ item }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = () => {
    dispatch(doneThunks.deleteItem(item.id)) // Adjust the thunk action as needed
      .unwrap()
      .then(() => {
        console.log(`Done item ${item.id} deleted successfully`);
        setShowConfirm(false); // Close modal on success
      })
      .catch((error) => {
        console.error("Failed to delete done item:", error);
      });
  };
  ...
  return (
    <>
      <div className="bg-white shadow-md p-4 rounded-lg cursor-pointer hover:shadow-lg transition relative group">
        ...
        <p className="text-green-600 font-bold">{item.status}</p>
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg flex items-center justify-center gap-4">
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(`/done/${item.id}/edit`); // Adjust the route as needed
            }}
            title="Edit"
          >
            {/* Edit Icon (Pencil) */}
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(`/dones/${item.id}/conversations`);
            }}
            title="Conversations"
          >
            <svg
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <p className="mb-6">
              Are you sure you want to delete "{item.doneTitle}"?
            </p>
...
export default DoneAccountCard;
